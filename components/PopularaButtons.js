...
const PopularaButtons = () => {
  return (
     ...
     <div style={{display:'flex',flexDirection:'column',justifyContent:'center',alignItems:'center',background:'#a100ff',height:'650px'}}>
         ...
         {servicesArrayPop &&    <div style={{width:'100%',display:'flex',flexDirection:'row',justifyContent:'center',alignItems:'center',flexWrap:'wrap',background:'#a100ff'}}>
    {servicesArrayPop?.map((el,i)=>(
        <Link href={el.path?el.path:'/'} key={i}  style={{borderRadius:'10rem',background:'#ff8be8',color:'#31024bff',margin:'0.5rem 0.5rem',display:'flex',alignItems:'center',padding:'0.75rem',textDecoration:'none'}}>
